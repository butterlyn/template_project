    ...
    def execute(self, data: dict) -> dict[str, Any]:
        ...
        for key, value in data.items():
            ...
            if isinstance(value, dict):
                items.extend(
                    FlattenDictCommand(new_key, sep=self._sep).execute(value).items()
                )
            else:
...
class DictionaryDeepestKeyValuePairsCommand(Command):
    ...
    def execute(self, data: dict) -> list:
        ...
        for key, value in data.items():
            (
                result.extend(
                    DictionaryDeepestKeyValuePairsCommand(
                        max_level=(self._max_level - 1)
                    ).execute(value)
                )
...
class SortDictByValuesCommand(Command):
    ...
    def execute(self, data: dict) -> dict:
        ...
        return dict(
            sorted(data.items(), key=lambda item: item[1], reverse=self._reverse)
        )
