...
def getRichLogger(
    ...
    def _convert_additional_handlers_to_list(
        additional_handlers: logging.Handler | Iterable[logging.Handler] | None,
        ...
        additional_handlers_list: list[logging.Handler] = (
            ...
            else [additional_handlers]
            if isinstance(additional_handlers, logging.Handler)
            # if additional_handlers provided as tuple or other non-list Iterable, convert to list:
            else list(additional_handlers)
            if isinstance(additional_handlers, Iterable)
            else TypeError(
                f"additional_handlers must be a logging.Handler, Iterable[logging.Handler], or None, not {type(additional_handlers)}"
            )
        )
    ...
    # Convert additional_handlers to list for combining with rich handler
    additional_handlers_list = _convert_additional_handlers_to_list(additional_handlers)

    # Combine the rich handler with any additional handlers
    all_handlers = (
        rich_handler + additional_handlers_list
        if enable_rich_logger
